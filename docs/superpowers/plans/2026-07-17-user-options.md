...
2. *Theme loads eagerly, not in `afterNextRender`.* Only unit prefs wait for `afterNextRender` (hydration safety for rendered text). The theme signal must initialize from storage in the service constructor: the `<html>` attribute is outside Angular's hydrated DOM, and waiting would let the effect strip the attribute the inline script stamped (dark flash). The spec's "adopts the stamped attribute" is implemented as "reads the same storage the script read, eagerly" — same result, testable.
